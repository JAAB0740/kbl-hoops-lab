  ...
  return (
    <div className="min-h-screen">
      <main className="mx-auto max-w-6xl px-6 py-8">
        ...
        <Link
          href="/games"
          className="mb-4 inline-flex items-center gap-1 text-[12px] text-ink-400 hover:text-ink-100"
        >
          ← 경기 목록
        </Link>
              ...
              {seriesGames.map((g, i) => {
                ...
                return (
                  <Link
                    ...
                    className={[
                      "flex items-center justify-between rounded-md border px-3 py-2 text-[12px] transition",
                      isCurrent
            ...
            <section className="card p-5">
              ...
              <div className="space-y-2">
                ...
                {homeRecent5.games > 0 && awayRecent5.games > 0 && (
                  <>
                    <div className="text-[10px] uppercase tracking-wider text-ink-500 mt-3 pt-2 border-t border-court-700/40">
                      최근 5경기 평균 (이 경기 직전)
                    </div>
                    <CompareRow
                      label="PPG"
          ...
          <section className="card p-5">
            ...
            <div className="mb-4 grid grid-cols-3 items-end gap-2">
              ...
              <div className="text-center text-[12px] text-ink-500">
                이전 H2H<br />
                {h2h.games.length === 0 ? (
                  <span className="text-[10px] text-ink-600">맞대결 없음</span>
                ) : (
                  <>({h2h.games.length}회)</>
                )}
              </div>
              ...
              {[...h2h.games].reverse().slice(0, 8).map((g, i) => {
                  ...
                  return (
                    <Link
                      key={i}
                      href={`/games/${gameToId(g)}`}
                      className="flex items-center justify-between rounded-md border border-court-700 bg-court-800/40 px-3 py-2 text-[12px] transition hover:border-court-600"
                    >
                      <span className="text-ink-400">
    ...
    <div className={align === "right" ? "md:text-right" : ""}>
      ...
      <div className="space-y-1.5">
        {items.map((it) => (
          ...
          >
            <span className="text-[10px] uppercase tracking-wider text-ink-500 w-16 shrink-0">
              {it.label}
            </span>
            {it.entry ? (
              <Link
...
function TeamHeader({
  ...
  return (
    <div className={align === "right" ? "text-right" : "text-left"}>
      ...
      <div className="mt-0.5 text-[12px] text-ink-400">{short}</div>
      {standing && (
        <div className="mt-2 stat-num text-[11px] text-ink-300">
          {standing.rank == null
            ? "개막 전"
...
function CompareRow({
  ...
  return (
    <div className="grid grid-cols-[60px_1fr_60px_1fr_60px] items-center gap-2 text-[11px]">
      <span
        className={[
  ...
  return (
    <div className="flex items-center justify-between gap-3">
      ...
      <div className="flex items-center gap-2">
        <span className="stat-num text-[11px] text-ink-400">
          {wins}-{losses}
        </span>
        <div className="flex gap-0.5">
          {form.length > 0 ? (
            ...
            <span className="text-[10px] text-ink-500">최근 폼 없음</span>
          )}
        </div>
      </div>
    </div>
