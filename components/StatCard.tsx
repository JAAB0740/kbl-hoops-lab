  ...
  return (
    <div className="group relative overflow-hidden card p-4 transition hover:border-court-600 hover:shadow-card-hover">
      ...
      <div className="flex items-center justify-between">
        <span className="text-[14px] font-medium uppercase tracking-[0.12em] text-ink-500">
          {label}
        </span>
      ...
      {caption && <div className="mt-2 text-[15px] text-ink-300">{caption}</div>}
    </div>
  );
}
...
function TrendIcon({ trend }: { trend: "up" | "down" | "flat" }) {
  ...
  const m = map[trend];
  return <span className={`text-[13px] font-semibold ${m.color}`}>{m.symbol}</span>;
}
