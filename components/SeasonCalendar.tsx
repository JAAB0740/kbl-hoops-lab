  ...
  if (months.length === 0 || !currentYm) {
    return (
      <div className="card p-8 text-center">
        <p className="text-[14px] text-ink-300">표시할 경기가 없어요.</p>
      </div>
    );
  }
      ...
      <div className="mb-4 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <button
            ...
            className="rounded-md border border-court-700 bg-court-800/70 px-2.5 py-1 text-[13px] text-ink-300 hover:border-court-600 hover:text-ink-100 disabled:opacity-30 disabled:cursor-not-allowed"
          >
            ← 이전
          </button>
      ...
      <div className="grid grid-cols-7 border-t border-court-700/60">
        {DOW_LABELS.map((d, i) => (
          <div
            key={d}
            className={[
              "border-b border-r border-court-700/60 px-2 py-1.5 text-[11px] uppercase tracking-wider",
              i === 0
                ? "text-buzzer-400"
                : i === 6
                  ? "text-hoop-400"
          ...
          return (
            ...
            >
              <div className="mb-1 flex items-center justify-between">
                <span
                  className={[
                    "stat-num text-[12px] font-semibold",
                    isToday
                      ? "text-flame-400"
                      : isWeekend
                        ? "text-ink-300"
...
function CalendarGameMini({
  ...
  return (
    <Link
      ...
      className={[
        "block rounded-sm px-1 py-0.5 text-[11px] ring-1 transition hover:bg-court-700/60",
        wlBg,
      ].join(" ")}
