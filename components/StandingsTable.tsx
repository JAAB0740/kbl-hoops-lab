            ...
            {data.map((t) => (
              ...
              >
                <td className="py-2.5 pl-3 align-middle">
                  <span className={`stat-num text-[13px] font-semibold ${t.accent}`}>
                    {t.rank}
                  </span>
                </td>
...
function TeamBadge({ code }: { code: string }) {
  return (
    <span className="flex h-7 w-7 items-center justify-center rounded-md border border-court-700 bg-court-900 text-[10px] font-bold tracking-wider text-ink-100">
      {code.slice(0, 3)}
    </span>
  );
}
...
function StreakChip({ streak }: { streak: string }) {
  ...
  return (
    <span
      className={[
        "inline-flex min-w-[2.25rem] justify-center rounded-md px-2 py-0.5 text-[11px] font-semibold stat-num",
        isWin
