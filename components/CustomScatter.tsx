...
function StatSelector({
  ...
  return (
    <div className="flex items-center gap-2">
      <label className="shrink-0 text-[10px] font-medium uppercase tracking-wider text-ink-500">
        {label}
      </label>
      <select
        value={value}
        onChange={(e) => onChange(e.target.value as StatKey)}
        className="flex-1 rounded-md border border-court-700 bg-court-800 px-2 py-1 text-[12px] text-ink-100 focus:border-flame-500 focus:outline-none"
      >
