...
export function CreatorCard() {
  return (
    <aside className="card overflow-hidden">
      ...
      <div className="border-t border-court-700/60 bg-court-900/40 px-5 py-3 text-[11px] text-ink-500">
...
function ExternalLink({
  ...
  return (
    ...
    >
      <span
        className={[
          "inline-flex h-6 w-6 items-center justify-center rounded border text-[12px] font-bold",
          color,
        ].join(" ")}
        aria-hidden
      >
      ...
      <span className="flex-1 min-w-0">
        <span className="block text-[13px] font-medium text-ink-100 group-hover:text-flame-400">
          {label}
        </span>
        <span className="block text-[11px] text-ink-500 truncate">{sub}</span>
      </span>
